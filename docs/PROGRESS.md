...
- 2026-09-20（边缘网关试错与回滚）：**边缘限流网关实测不可行，方案 B 落地**（20b11ac/8026bd2/6b8472b 回滚 + 本提交）——曾实现 /api/gw 边缘网关（KV 限流 + 令牌黑名单 + 跨域回源代理），实测发现 **EdgeOne 边缘函数 fetch 子请求不进函数路由**（同域落静态层返回 SPA；blacklist 固定路径端点正常、catch-all 代理始终返回 SPA），代理架构不可行。回滚网关与前端切换，改为：①限流落 Node 侧（lib/auth.js rateLimit 内存固定窗口，市面常见频率：登录 IP+账号 5/min + IP 30/min、刷新 30/min、忘记密码 3/hour）；②边缘价值改用原生轻端点体现：新增 /api/edge/stats（KV 访问统计，无 DB、边缘毫秒级），HomeView 挂 fire-and-forget 埋点；③HomeView 模块卡更新至 M3 现状（图书/失物/社团/论坛上线，移除二手集市）；④架构文档同步新口径（铁律 HANDOVER #23/#24、ARCHITECTURE 2.5 补平台行为结论）
