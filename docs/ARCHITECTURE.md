...
- **接口限流落在 Node 侧且必须走 DB 流水计数**（多实例内存不共享，内存计数实测无效）：登录用 sys_login_log 失败流水（IP+账号 5 失败/min 防撞库、IP 30 失败/min 防爆破）、忘记密码用 sys_email_code（3 次/hour/IP）、注册沿用 DB 频控——均为市面常见频率；KV 60s 最终一致同样不适合做精确限流计数
